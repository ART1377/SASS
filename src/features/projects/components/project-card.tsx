...
export function ProjectCard({ project, onEdit, onDeleteRequest, onInvite }: ProjectCardProps) {
  const actions = [
    { label: 'ویرایش', icon: Pencil, onClick: () => onEdit(project) },
    { label: 'دعوت عضو', icon: UserPlus, onClick: () => onInvite(project.id) },
    { label: 'حذف', icon: Trash2, onClick: () => onDeleteRequest(project.id), destructive: true },
  ];
